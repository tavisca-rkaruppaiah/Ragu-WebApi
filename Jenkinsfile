pipeline {
  ...
  stages {
        stage('Pull from Docker') {
            steps {
                    powershell('docker pull ${env:USER_NAME}/${env:REPOSITORY_NAME}:${env:TAG_NAME}')
                    powershell("echo pulling........")
              }
          } 
        stage('Run') {
            steps {
                    powershell('docker run -p ${env:PORT}:80 ${env:USER_NAME}/${env:REPOSITORY_NAME}:${env:TAG_NAME}')
                    powershell("echo Running.......")
            }
        }
